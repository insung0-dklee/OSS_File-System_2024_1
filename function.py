...
def fileNamer():
    # 현재 폴더의 파일 목록을 가져와서 사용자에게 출력
    files = os.listdir('.')
    print("현재 폴더의 파일 목록 :\m")
    for index, file in enumerate(files, 1):
        print(f"{index}. {file}")
    # 이름을 변경할 파일들을 선택해 리스트에 저장
    indexList = []
    for x in input("""변경할 파일들의 번호를 변경하고 싶은 순서대로, 공백 문자 혹은 -로 구분해 입력 
        ...
        if re.match(r'^\d+$|^\d+-\d+$', x):
            ...
            return
    # 파일 인덱스가 중복되는지 검사하고 인덱스가 중복되지 않으면 리스트에 파일 목록 저장
    ...
    for x in renameList:
        print(x)
    # 변경할 이름을 설정
    # 파일 목록을 확인하고 취소할 경우 -1을 입력해 중단 가능
    ...
    if new_filename=="-1":
        ...
        return
    # 파일 이름 변경
    # 파일 이름을 찾을 수 없거나 파일이 이미 존재할 경우 오류 메세지 출력
    else:
        try:
            for rename in range(len(renameList)):
                _, file_extension = os.path.splitext(renameList[rename])
                
                os.rename(renameList[rename], new_filename + '_' +str(rename+1).rjust(3, '0') + file_extension)
            print(f"파일이 성공적으로 변경되었습니다.")
