...
def compress_file(file_path):
    """
    사용자가 파일 경로를 입력하면 해당 파일을 zip으로 압축합니다.

    매개변수:
    file_path (str): 압축할 파일의 경로
    """
    try:
...
def list_file_creation_times(directory):
    ...
    매개변수:
    directory (str): 파일 생성 시간을 출력할 디렉토리의 경로
    """
    try:
...
def preview_image(file_path):
    ...
    매개변수:
    file_path (str): 미리보기할 이미지 파일의 경로
    """
    try:
