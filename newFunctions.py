...
def compress_file(file_path):
    ...
    """
    try:
        file_dir = os.path.dirname(file_path)  # 파일이 있는 디렉토리 경로
        file_name = os.path.basename(file_path)  # 파일 이름 추출
        output_zip = os.path.join(file_dir, f"{file_name}.zip")  # 압축 파일 경로 설정

        with zipfile.ZipFile(output_zip, 'w') as zipf:
            zipf.write(file_path, file_name)  # 파일을 zip으로 압축
...
def preview_image(file_path):
    ...
    """
    try:
        with Image.open(file_path) as img:  # 이미지 파일 열기
            plt.imshow(img)  # 이미지를 화면에 표시
            plt.axis('off')  # 축을 끔
            plt.show()  # 이미지를 표시하는 창 열기
    except Exception as e:
...
def preview_text(file_path, num_lines=5):
    ...
    """
    try:
        with open(file_path, 'r', encoding='utf-8') as file:  # 텍스트 파일 열기
            lines = [next(file) for _ in range(num_lines)]  # 처음 몇 줄 읽기
        for line in lines:  # 읽은 줄 출력
            print(line, end='')
    except Exception as e:
