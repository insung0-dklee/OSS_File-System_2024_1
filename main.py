...
def showFavorites():
    if not favorites:
        ...
        for i, favorite in enumerate(favorites, 1):
            print(f"{i}. {favorite}")
# 압축 기능
def zip_files(file_paths, archive_path, format, password=None):
    try:
        if format == 'zip':
            ...
            with tarfile.open(archive_path, f"w:{format}") as tarf:
                for file in file_paths:
                    if os.path.isfile(file):
            ...
            print("지원하지 않는 압축 형식입니다.")
            return
        print(f"파일이 {archive_path}로 압축되었습니다.")
    except PermissionError:
        print(f"오류: {archive_path}에 대한 권한이 거부되었습니다. 권한을 확인하세요.")
    except Exception as e:
        print(f"파일 압축 중 오류가 발생했습니다: {e}")

# 압축 해제 기능
def unzip_file(archive_path, extract_to, format, password=None):
    try:
        if format == 'zip':
            ...
            with tarfile.open(archive_path, f"r:{format}") as tarf:
...
while not b_is_exit:
    ...
    if func == "1":
        ...
        archive_path = input("생성할 압축 파일의 경로를 입력하세요: ")
        format = input("압축 형식을 선택하세요 (zip, tar, gztar, bztar, xztar): ")
        password = input("압축 파일에 설정할 비밀번호를 입력하세요 (비워두면 비밀번호를 설정하지 않음): ")
        password = password if password else None
        zip_files(file_paths, archive_path, format, password)

    elif func == "압축해제":
        archive_path = input("압축 해제할 파일의 경로를 입력하세요: ")
        extract_to = input("파일을 추출할 경로를 입력하세요: ")
        format = input("압축 형식을 선택하세요 (zip, tar, gztar, bztar, xztar): ")
        password = input("압축 파일의 비밀번호를 입력하세요 (비밀번호가 없으면 비워두기): ")
        password = password if password else None
        unzip_file(archive_path, extract_to, format, password)
